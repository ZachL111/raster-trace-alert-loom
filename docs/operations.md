...
`raster-trace-alert-loom` is centered on observability work in Scala. The technical target is: Package a Scala local lab for alert analysis with safe and unsafe fixtures, remediation hints, and documented operating limits.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 5.
